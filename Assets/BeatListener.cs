...
public class BeatListener : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            objsLColor[i] = objsL[i].GetComponent<SpriteRenderer>().color;
        }
    ...
    void UpdateBeat()
    {
        ...
        {
            float sumtmp = Mathf.Clamp(sum, 1f, 1.1f);
        ...
        for (int i = 0; i < objsL.Length; i++)
        {
            ...
            var sr = objsL[i].GetComponent<SpriteRenderer>();
                
            sr.color = new Color(objsLColor[i].r*sumtmp, objsLColor[i].g*sumtmp, objsLColor[i].b*sumtmp, sr.color.a);
        }
    }
}
